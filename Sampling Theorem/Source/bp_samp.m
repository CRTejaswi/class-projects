...
%function bp_samp(fl,fh,fs,fmax,N,shape)
% =========================================================================
%               Bandpass Sampling Theorem Plotting Function
% =========================================================================
% fl = spectrum lower frequency in Hz
...
% spectral shape taken from the function bp_tri().
...
plot(f,bp_tri(f,fl,fh,shape),'k');
% overlay positive and negative frequency translates
hold
for n = 1:N
    plot(f,bp_tri(f-n*fs,fl,fh,shape),':r');
    plot(f,bp_tri(f+n*fs,fl,fh,shape),':g');
end
hold
title('Bandpass Sampling Theorem for a Real Signal: Blk = orig, dotted = translates')
ylabel('Spectrum Magnitude')
xlabel('Frequency in Hz')
function x = bp_tri(f, fl, fh, shape)
